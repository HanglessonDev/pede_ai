...
class TestNovasConsultas:
    ...
    def test_extracoes_sem_itens(self, tmp_path) -> None:
        ...
        with open(csv_path, 'w', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
            writer.writerow(['timestamp', 'thread_id', 'mensagem', 'itens_encontrados', 'itens_ids', 'variantes_encontradas', 'tempo_ms'])
            writer.writerow(['2026-01-01', 's1', 'quero algo estranho', '0', '', '', '12.50'])
            writer.writerow(['2026-01-02', 's2', 'quero hamburguer', '1', 'lanche_001', 'simples', '8.30'])
    ...
    def test_handlers_com_erro(self, tmp_path) -> None:
        ...
        with open(csv_path, 'w', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
            writer.writerow(['timestamp', 'thread_id', 'handler', 'intent', 'input_resumo', 'output_resumo', 'tempo_ms', 'erro'])
            writer.writerow(['2026-01-01', 's1', 'handler_pedir', 'pedir', '{}', '{}', '5.0', 'KeyError'])
            writer.writerow(['2026-01-02', 's2', 'handler_pedir', 'pedir', '{}', '{}', '3.0', ''])
    ...
    def test_tempo_medio_handlers(self, tmp_path) -> None:
        ...
        with open(csv_path, 'w', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
            writer.writerow(['timestamp', 'thread_id', 'handler', 'intent', 'input_resumo', 'output_resumo', 'tempo_ms', 'erro'])
            writer.writerow(['2026-01-01', 's1', 'handler_pedir', 'pedir', '{}', '{}', '10.0', ''])
            writer.writerow(['2026-01-02', 's2', 'handler_pedir', 'pedir', '{}', '{}', '20.0', ''])
    ...
    def test_funil_com_abandono_sem_filtro(self, tmp_path) -> None:
        ...
        with open(csv_path, 'w', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
            writer.writerow(['timestamp', 'thread_id', 'etapa_atual', 'intent', 'carrinho_size'])
